...
## 2026-08-22 TikTok真实只读同步与1688精确绑定

### TikTok官方数据实测

Cloudflare连接器健康状态为 `ok`，授权令牌与店铺均已保存。隔离的只读密钥成功读取2家授权店铺：菲律宾和泰国。官方商品汇总如下：

| 市场 | 商品总数 | 激活 | 草稿 | 停用/删除/失败 | 库存为0 |
|---|---:|---:|---:|---:|---:|
| 菲律宾 PH | 53 | 42 | 9 | 2 | 2 |
| 泰国 TH | 60 | 44 | 1 | 15 | 1 |

本地n8n生产Webhook `tiktok-shop-products` 已真实返回官方商品，输出明确标记 `mode=read_only` 和 `mutation_routes_enabled=false`。随后使用一次性工作台数据库完成真实链路：
...
菲律宾同步53件、泰国同步60件，工作台共保存113件。测试数据库在验证后删除，没有把真实商品副本写入尚未设置老板账号的正式本地数据库。

实测发现TikTok审核成功状态为大写 `APPROVED`。旧规则只识别 `approved` 和 `AUDIT_STATUS_APPROVED`，导致113件商品全部误报异常。现已统一对商品和审核状态执行大写归一化；修复后27件商品存在至少一项真实规则异常，异常事件包括：库存为0共3次、删除13次、草稿10次、失败2次、卖家停用2次、审核失败2次、审核状态NONE 23次。同一商品可以同时有多项异常，因此事件数会大于异常商品数。

### 1688精确商品与SKU绑定

系统现在强制执行以下供应链身份规则：

1. 只接受 `https://detail.1688.com/offer/数字ID.html` 或 `https://m.1688.com/offer/数字ID.html`；
2. OFFER ID必须是5到30位数字，且必须与链接路径中的数字完全相同；
3. 保存时删除查询参数和锚点，形成规范化来源链接；
4. 快照同时包含供应商ID、供应商名称、OFFER ID、规范化链接、采购价、币种、库存、发货天数、准确供应商SKU ID和非空SKU属性；
5. 原始采集时间写入 `verified_at`；超过7天的快照不能提交或批准上架；
6. 供应商库存为0时不能提交或批准上架；
7. 采购前必须重新提交价格、库存、时效和全部SKU映射，并与原快照哈希完全一致；
8. 任一字段变化返回冲突并阻止采购；同一订单与快照重复请求使用相同幂等记录；
9. 采购队列仍标记 `requires_human_payment=true`，没有自动付款入口。

PostgreSQL真实HTTP验收已通过：精确供应商身份、选品证据门槛、人工审批待执行项、采购待执行项、采购幂等和跨店403均正确。当前21项自动回归通过，实际TikTok写入为0，采购付款为0。结构化证据：`demo-evidence/selection-radar/2026-08-22/tiktok-readonly-and-exact-supplier-binding.json`。

### 仍未完成

- `/api/internal/history` 仍因本地内部巡检密钥与生产密钥不匹配返回401；它不影响当前隔离只读密钥读取店铺与商品，但D1同步历史仍未验证。
- 本机Wrangler配置均未登录Cloudflare，因此不能安全读取生产D1或旋转内部密钥；不能盲目覆盖生产密钥，以免破坏其他内部客户端。
